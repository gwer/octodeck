/**
 * Source:
 * https://github.com/mourner/yeahml/blob/915b2a504833e0755bf7732040e90c3890210430/index.js
 */
'use strict';
...
function tokenize(s) {
  ...
  while (pos <= len) {
    ...
    if (c === HASH) {
      ...
      // line break; save indent
      while (pos < len && s.charCodeAt(pos) === SPACE) pos++;
      indent = pos - start - 1;
    } else if (c === SPACE) {
      // spaces; skip
      while (pos < len && s.charCodeAt(pos) === SPACE) pos++;
    } else if (c === HYPHEN && s.charCodeAt(pos) === SPACE) {
      // "- ": sequence entry
      pos++;
...
function parseTokens(s, tokens) {
  ...
  function expect(expectedType) {
    if (!accept(expectedType))
      throw new Error(
        `Expected ${types[expectedType]}, got ${types[nextType]}.`,
      );
  }
